...
util.extend(Zombie.prototype, {
    ...
    _renderNode: function _renderNode(name, props) {
        ...
        nodeProps = node.properties = node.properties || {};
        ...
        if (layout instanceof Node) {
            if ("varname" in node) {
                nodeProps[node.varname] = nodeRendered;
                if ("content" in nodeProps) {
                    if ("content" in node.properties) {
                        nodeProps.content = node.properties.content;
                    } else {
                        delete nodeProps.content;
                    }
                }
            } else {
                nodeProps.content = nodeRendered;
            }
